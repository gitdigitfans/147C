// fix-bedroom-images-v2.mjs
// Resume-safe: only targets images still pointing to pharaohfurniture.com
...
async function main() {
  console.log("Fetching remaining broken bedroom images from D1...");
  ...
  console.log(`Found ${rows.length} remaining images to migrate.\n`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (failed <= 10) console.error(`  FAIL [${row.id}] ${row.slug}: ${err.message}`);
    }
  ...
  if (errorLog.length > 0) {
    fs.writeFileSync("scripts/fix-bedroom-errors.json", JSON.stringify(errorLog, null, 2));
